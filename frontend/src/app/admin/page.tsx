...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (!loading && user && user.role !== "ADMIN") { router.push("/dashboard"); toast("Accès admin requis", "error"); }
  ...
  async function handleChangePlan(userId: string, plan: string) {
    ...
    const token = localStorage.getItem("fm_token") || "";
    try {
      await adminFetch(`/admin/users/${userId}/plan`, token, {
        method: "PUT",
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-dark-700 text-gray-500 text-xs">
                    ...
                    <th className="text-left px-4 py-3">LLM auj.</th>
                    <th className="text-left px-4 py-3">Rendus auj.</th>
                    ...
                    <tr key={u.id} className="border-b border-dark-700/50 hover:bg-dark-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          className={`text-xs px-2 py-0.5 rounded border-0 outline-none cursor-pointer disabled:opacity-50 ${
                            u.plan === "PRO" ? "bg-brand-600 text-white" :
                            u.plan === "ENTERPRISE" ? "bg-purple-600 text-white" :
                      ...
                      <td className="px-4 py-3 text-gray-400">{u.quota?.llmCallsToday ?? 0} / {u.quota?.llmCallsLimit ?? "-"}</td>
                      <td className="px-4 py-3 text-gray-400">{u.quota?.rendersToday ?? 0} / {u.quota?.rendersLimit ?? "-"}</td>
